...
const randomPerson = TestDataGenerator.generatePerson()
const randomAddress = TestDataGenerator.generateAddress()
...
test(`Ordering a British Shorthair Cat`, async ({page, SogetiPetStore }) => {
  ...
  await test.step('Add Personal Details to order and set billing and shipping adress to the same adress', async () => {
    await SogetiPetStore.checkoutPage.setPersonalDetails(randomPerson.Name, randomPerson.lastName, randomPerson.email, randomPerson.phone)
    await SogetiPetStore.checkoutPage.setAddressDetails(randomAddress.street, randomAddress.city, randomAddress.postalCode, randomAddress.country, randomAddress.province)
